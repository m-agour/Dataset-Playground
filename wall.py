...
from helpers import get_contours, draw_contours, cluster_points, imshow

from sklearn.cluster import MeanShift, estimate_bandwidth


def get_wall_width(r, g, b, threshold=0):
    ...
    # imshow(voimg)
    return round(np.median([c[2][3] for c in contours_v] + [c[2][2] for c in contours_h]))
...
#
#     f = fm
#     f = cv2.erode(f, np.ones((mwallwidth // 2, mwallwidth // 2), np.uint8), iterations=1)
